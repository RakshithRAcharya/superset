...
class CustomAuthDBView(AuthDBView):
    ...
    def register(self):
        ...
        # ----------- AWS RDS ------------- #
        # conn = connect(user='postgres', password='password', host='######.rds.amazonaws.com', port='5432')
        # conn.set_isolation_level(0)
        # cur = conn.cursor()
        # statement = "CREATE DATABASE {0} WITH OWNER = postgres ENCODING = 'UTF8' LC_COLLATE = 'en_US.UTF-8' LC_CTYPE = 'en_US.UTF-8' CONNECTION LIMIT = -1;".format(username)
        ...
        # ------------IBM DB--------------- #
        # conn = connect( host="##########.databases.appdomain.cloud", # TryDB
        #                 port= 30876, 
        #                 user="username", 
        #                 password="passwodr", 
        #                 sslmode="verify-full", 
